...
// 从环境变量 jywip 读取内网 IP，多个 IP 用换行分隔
const SERVERS = (process.env.jywip || "")
    .split(/\r?\n/)
    .map(s => s.trim())
    .filter(Boolean);

if (!SERVERS.length) {
    console.log("❌ 未配置环境变量 jywip，请设置后重试");
    console.log("格式示例：");
    console.log("  jywip=127.0.0.1:8088");
    console.log("  或");
    console.log("  jywip=127.0.0.1:8088\\n192.168.31.36:8088\\n192.168.31.88:8088");
    process.exit(1);
}
...
async function getCode(server) {
    ...
    console.log(`🔐 [授权] 请求本地 code 服务: ${url}`);
    ...
    try {
        ...
        if (res.data?.err !== 0 || !res.data?.code) {
