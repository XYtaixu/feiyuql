...
环境变量：
  ...
  jywip             内网wxcode服务地址，多个换行分隔，格式：192.168.1.21:8088
"""
...
def get_code(server: str) -> str | None:
    ...
    try:
        ...
        if data.get("err") != 0 or not data.get("code"):
            print(f"[{server}] 获取 code 失败：{json.dumps(data, ensure_ascii=False)}")
            return None

        print(f"[{server}] 获取 code 成功")
        return data["code"]
    except Exception as exc:
        print(f"[{server}] 获取 code 异常：{exc}")
        return None
...
def main() -> None:
    print("===== 奈雪点单动态code签到（jywip多内网+品赞代理+PushPlus）=====\n")
