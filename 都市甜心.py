...
流程：
  1. 本地服务获取微信 code（从环境变量 jywip 读取，换行分隔多地址）
...
def get_code(server: str) -> str | None:
    ...
    log_step("🔐", "授权", f"请求本地 code 服务 {url}")

    try:
        res = direct_session().get(url, params={"appId": APPID}, timeout=20)
        data = res.json()

        if data.get("err") != 0 or not data.get("code"):
            log_error("授权", f"code 获取失败：{json_preview(data)}")
            return None

        log_success("授权", "code 获取成功")
        return data["code"]
    except Exception as exc:
        log_error("授权", f"code 获取异常：{exc}")
        return None
