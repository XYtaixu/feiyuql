...
# ===================== 配置项 =====================
# 从环境变量 jywip 读取wxcode服务地址，多行换行分隔
SERVERS = []
...
# 无有效地址直接退出
if len(SERVERS) == 0:
    print("❌ 错误：未读取到环境变量 jywip 或无有效IP端口！")
    print("青龙环境变量jywip填写示例（每行一个地址）：")
    ...
    async def get_code(self) -> Optional[str]:
        ...
        for retry in range(3):
            try:
                ...
                if response.status_code != 200:
                    print(f"⚠️ [{self.server}] 第{retry+1}次获取code失败 | HTTP错误: {response.status_code}")
                    await sleep(1000)
                    continue

                res = response.json()
                if res.get("err") != 0 or not res.get("code"):
                    print(f"⚠️ [{self.server}] 第{retry+1}次获取code失败 | 业务错误: {res.get('msg', '未知错误')}")
                    await sleep(1000)
                    continue
                ...
                print(f"⚠️ [{self.server}] 第{retry+1}次获取code失败 | 响应不是JSON格式")
                await sleep(1000)
            except Exception as e:
