...
# ===================== 业务函数 =====================
def getCode(server):
    requestUrl = f"http://{server}/login"
    print(f"📡 [{server}] 正在请求登录服务：{requestUrl}")
    try:
        res = requests.get(
            requestUrl,
            params={"appId": APPID},
        ...
        res_data = res.json()
        if res_data.get("err") != 0 or not res_data.get("code"):
            print(f"❌ [{server}] 返回异常: {res.text[:100]}")
            return None
...
# ===================== 主程序 =====================
async def main():
    print('===== 三福动态code签到（环境变量jywip多内网+品赞代理+sid鉴权修复版）=====\n')
