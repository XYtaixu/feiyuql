...
def get_wx_code(code_url):
    """获取对应接口的Code【强制直连，不走代理】"""
    try:
        res = requests.get(
            code_url,
            params={"appId": APP_ID},
            timeout=20,
            proxies={"http": None, "https": None}  # 核心：强制直连
        )
        data = res.json()
        if data.get("err") == 0:
            print("✅ 获取Code成功")
...
def refresh_token(code_url, proxy_config, account_name):
    ...
    try:
        payload = {"code": code, "appId": APP_ID, "client": "wxmp"}
...
if __name__ == "__main__":
    print('===== DT生活签到（环境变量jywip读取内网多服务+独立代理版）=====\n')
