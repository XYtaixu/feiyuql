...
# 1. ❌ 获取code失败
#    - 检查code服务是否正常运行
#    - 确认jywip内服务器地址和端口是否正确
#    - 测试浏览器访问：http://你的服务器地址/login?appId=wxc5db704249c9bb31
#
# 2. ❌ 获取token失败
#    - 确认code未过期（有效期5分钟）
...
# ===================== 配置项 =====================
# 从环境变量 jywip 读取内网wxcode服务地址，多条换行分隔
SERVERS = []
...
# 无有效地址直接退出并提示
if len(SERVERS) == 0:
    print("❌ 错误：未读取到环境变量 jywip 或无有效服务地址！")
    print("配置示例（青龙环境变量jywip值，每行一个地址）：")
...
class QueChaoBot:
    ...
    async def get_code(self) -> Optional[str]:
        """从本地服务获取code（青龙优化版日志）"""
        request_url = f"http://{self.server}/login?appId={APPID}"
        print(f"📡 [{self.server}] 请求登录接口 | URL: {request_url}")

        try:
            async with httpx.AsyncClient(timeout=20.0, transport=build_direct_transport()) as client:
                response = await client.get(request_url)

            print(f"📝 [{self.server}] 接口响应 | 状态码: {response.status_code}")

            if response.status_code != 200:
                print(f"❌ [{self.server}] 获取code失败 | HTTP错误: {response.status_code}")
                return None

            res = response.json()
            if res.get("err") != 0 or not res.get("code"):
                print(f"❌ [{self.server}] 获取code失败 | 业务错误: {res.get('msg', '未知错误')}")
                return None

            # 只打印code前8位，隐藏敏感信息
            code_preview = res["code"][:8] + "..."
            print(f"✅ [{self.server}] 获取code成功 | 预览: {code_preview}")
            return res["code"]

        except json.JSONDecodeError:
            print(f"❌ [{self.server}] 获取code失败 | 响应不是JSON格式")
            return None
        except Exception as e:
            print(f"❌ [{self.server}] 获取code异常 | 原因: {str(e)}")
            return None
